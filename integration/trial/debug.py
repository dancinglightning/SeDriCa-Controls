...
rospy.Subscriber("/A_star_path",Path,path_callback,x_ini)
rospy.Subscriber("/velocity_array",Float64MultiArray,v_callback)
rospy.spin()
'''global velocities
points=[]
velocities=[1.5]
...
def callback(path,v_arr):
	...
	if len(points):
		for i in range(l):
			if points[i][0]!=path.poses[i].pose.position.y:
				print("##########")
			if i==l-1:
				path_repeat=True
	if not path_repeat:
...
vel_sub=message_filters.Subscriber("/velocity_array",Float64MultiArray)
